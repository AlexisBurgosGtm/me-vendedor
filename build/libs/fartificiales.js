...
function write_fire() {
    var b, s;
    b=document.createElement("div");s=b.style;s.position="absolute";
...
function bang() {
    var i, X, Y, Z, A=0;
    for (i=0; i<bits; i++) {X=Math.round(Xpos[i]);Y=Math.round(Ypos[i]);
...
function stepthrough() {
    ...
    Z.visibility="visible";} bang();launch();}
...
var s=document.getElementById("bod").style;s.top=sdown+"px";s.left=sleft+"px";}
...
window.onload=function() { 
    if (document.getElementById) {
        ...
        write_fire();
        launch();
        timerEfecto = setInterval('stepthrough()', speed);
    }
}
//]]>